/**
 * @file
 * Javascript for slackposter.
 */

/**
 * Keeps track of the button id if multiple buttons created.
 *
 * @type int
 */
var button_count = 1;
...
var timeout = null;
/*
    These functions create buttons to launch various types of slack input
    screens.
    Returns the button id.
    The button is appended to the anchor supplied and is themeable, deployed in
    a <div> slackLaunchWrapper and the <input type=button> has the id as
    returned and a class slackLaunch
 */
...
function tm_slack_add_button(parent, buttonTxt, channel, user, note){
  ...
  var id = "slackButton" + button_count;
  $(parent)
    .append($("<div id='" + id + "Wrapper" + button_count + "'>")
      .addClass('slackLaunchWrapper')
      .attr({"title":"Post to Slack"})
      .append($("<input type='submit' id='" + id + "'>")
        .addClass("slackLaunch")
        .attr("value",buttonTxt)
      )
    );
...
function tm_slack_add_anchor(parent, buttonTxt, channel, user, note){
    ...
    user = typeof user !== 'undefined' ? user : null;
    var id = "slackButton" + button_count;
    $(parent).append($("<div id='" + id + "Wrapper" + button_count + "'>").addClass('slackLaunchWrapper').attr({"title":"Post to Slack"})
...
/*
 * If channel and user are null or not supplied, then the defaults for the
 * channel will be used and user will not be able to change them.
 * If channel and user are empty strings, then the user can specify them.
 * If channel and user are supplied then they will be used and user will not
 * be able to change them.
*/
function tm_slack_support_popup(channel, user, note, value){
  // First make the usual popup.
  tm_slack_popup(channel, user, note);
  ...
  $("#commentWrapper")
    .before($("<div id='productWrapper'>").addClass("slack-form-wrapper")
  ...
  $("#commentWrapper")
    .before($("<div id='subjectWrapper'>").addClass("slack-form-wrapper")
...
function tm_slack_popup(channel, user, note){
  ...
  if (channel !== "" && channel !== null) {
    $("#slackPop").append($("<input type='hidden' id='channel'>").val(channel));
  }
  else if (channel === "") {
    // If channel is null, then this wont show and the default for this
    // webhook will be used.
    $("#slackPop")
      .append($("<div id='channelWrapper'>")
        .addClass("slack-form-wrapper")
        .append($("<label>").html("Channel:"))
        .append($("<input type='text' id='channel'>")
          .addClass("slack-form-item")
        )
      );
  }
  // If channel is provided, then that value is used but hidden.
  if (user !== "" && user !== null) {
    $("#slackPop").append($("<input type='hidden' id='user'>").val(user));
  }
  else if (user === "") {
    // If no user is provided, then this wont show and the default for this
    // module (set in SlackPost()) will be used.
    $("#slackPop")
      .append($("<div id='userWrapper'>")
        .addClass("slack-form-wrapper")
        .append($("<label>").html("Post Author:"))
        .append($("<input type='text' id='user'>")
          .addClass("slack-form-item")
        )
      );
  }
  ...
  // Make the popup.
  $("#slackPop")
    .append($("<div id='commentWrapper'>").addClass("slack-form-wrapper")
      .append($("<label>").html("Posting:"))
      .append($("<textarea id='comment'>")
        .addClass("slack-form-item")
      )
    )
...
function validate(type){
  ...
  if (!result) {
    $("#slack_message").html("Require highlighted information.").addClass("err").show();
  }
  return result;
}
...
function tm_slack_post(comment, channel, user){
  // If channel is not provided, then it will post to the default set in
  // Slack for this webhook.
  user = typeof user !== 'undefined' ? user : 'Website User';
